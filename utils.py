...
def get_label(audio_config):
    """Tra ve nhan tuong ung voi dac diem duoc trich xuat
    Vi du:
...
def extract_feature(file_name, **kwargs):
    ...
    try:
        ...
        if v:
            raise NotImplementedError("Chuyen doi khong dung, neu khong duoc hay tai ffmpeg va cai trong may tinh o trong Path.")
    else:
...
def get_best_estimators(classification):
    """
    Tai cac cong cu uoc tinh trong thu muc 'grid'. Neu muon dung nhieu cai khac nhau, co the tinh chinh cac tham so trong 'grid_search.py' va chay (mat nhieu gio de hoan thanh no)
    """
    if classification:
...
def get_audio_config(features_list):
    """
    Chuyen doi danh sach dac diem (feature) thanh tu dien de hieu tu loai
    ...
    for feature in features_list:
        if feature not in audio_config:
            raise TypeError(f"Dac diem duoc dua vao: {feature} khong duoc chap nhan.")
